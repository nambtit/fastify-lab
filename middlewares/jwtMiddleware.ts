...
export async function jwtMiddleware(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    if (!authHeader) {
      return sendError(reply, 'Missing Authorization header', StatusCodes.UNAUTHORIZED);
    }
    const parts = authHeader.split(' ');
    if (parts.length !== 2 || parts[0] !== 'Bearer') {
      return sendError(reply, 'Invalid Authorization header format', StatusCodes.UNAUTHORIZED);
    }
    const token = parts[1];
    ...
    if (!nbf || Date.now() < (nbf as number)) { 
      throw new Error('Access token not yet valid');
    }

    if (!exp || Date.now() > (exp as number)) {
